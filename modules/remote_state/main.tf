...
resource "null_resource" "post-deploy" {
  ...
  provisioner "local-exec" {
   command = "echo 'storage_account_name = ${azurerm_storage_account.stacc.name}' > ${var.sas_output_file}"
  }
  provisioner "local-exec" {
   command = "echo 'container_name = tf-state' >> ${var.sas_output_file}"
  }
  provisioner "local-exec" {
    command = "echo 'key = terraform.tfstate' >> ${var.sas_output_file}"
  }
  provisioner "local-exec" {
  command = "echo 'sas_token = \"${data.azurerm_storage_account_sas.state.sas}\"' >> ${var.sas_output_file}"
  }
}
